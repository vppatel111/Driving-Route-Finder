// Completed srv_path_len, srv_get_waypoints
...
int16_t srv_get_pathlen(LonLat32 start, LonLat32 end) {
    ...
    // Processes the number of waypoints.
...
int16_t srv_get_waypoints(LonLat32* waypoints, int16_t path_len,
    ...
    for (int16_t i=0; i < path_len; ++i) {
      ...
      if (path_len == max_path_len) { // Skip if too many waypoints
        continue;
      }

      // Processes latitude first
    ...
    // Makes sure we get the final end of waypoints, ("E\n").
    if (strcmp(field2, validCommand)) {
      return 1;
    } else {
...
/*
    ...
    timeout - The amount of time in milliseconds the client will wait for
